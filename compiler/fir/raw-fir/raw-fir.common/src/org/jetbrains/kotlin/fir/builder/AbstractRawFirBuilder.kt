    ...
    private fun generateIncrementOrDecrementBlockForArrayAccess(
        ...
        return buildBlockPossiblyUnderSafeCall(
            ...
            fun buildSetCall(argumentExpression: FirExpression, sourceElementKind: KtFakeSourceElementKind) = buildFunctionCall {
                ...
                // It's important to preserve `propertyAccessSource` as `FirElementRecorder` relies on these
                // sources, and otherwise it would be harder to fix it to account for the new ones.
                explicitReceiver = generateResolvedAccessExpression(
                    propertyAccessSource = arrayVariable.source,
                    variable = arrayVariable,
                    calleeReferenceSource = wrappedReceiver?.toFirSourceElement(sourceKind),
                )
                argumentList = buildArgumentList {
                    for (indexVar in indexVariables) {
                        arguments += generateResolvedAccessExpression(indexVar.source, indexVar)
